class MenuView extends AbstractCanvasView {
     ...
     * 视图持有对其控制器的引用
     */
    private viewController: ReversiViewController
    ...
    static readonly touchedButtonAlpha = 0.8
    private buttonGap: number
    private button1: TwoHalfCircle
    private button2: TextCircle
    ...
    private _witheChessCount = 0
    get witheChessCount(): number {
        return this._witheChessCount
    }
    set witheChessCount(x: number) {
        this._witheChessCount = x
        this.button3.number = this._witheChessCount
        this.redrawButton3()
    }

    private _blackChessCount = 0
    get blackeChessCount(): number {
        return this._blackChessCount
    }
    set blackChessCount(x: number) {
        this._blackChessCount = x
        this.button2.number = this._blackChessCount
        this.redrawButton2()
    }

    constructor(width: number, height: number, viewController: ReversiViewController) {
    ...
    private drawButtons() {
        ...
        this.button2 = new TextCircle(centerX1 + this.buttonGap, centerY, radius, 2) // 最后一个数字2是textcircle中初始化的数字
        this.button3 = new TextCircle(centerX1 + this.buttonGap * 2, centerY, radius, 2)
        this.button4 = new Circle(centerX1 + this.buttonGap * 3, centerY, radius)
        
        this.button2.fill = true
        this.button2.fillColor = "gery"

        this.button4.fill = true
        this.button4.fillColor = "green"
